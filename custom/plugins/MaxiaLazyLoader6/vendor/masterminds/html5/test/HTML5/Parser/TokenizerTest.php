...
class TokenizerTest extends \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\TestCase
{
    // ================================================================
    ...
    {
        $this->assertEquals($type, $event['name'], "Event {$type} for " . \print_r($event, \true));
        if (\is_array($expects)) {
            $this->assertEquals($expects, $event['data'], "Event {$type} should equal " . \print_r($expects, \true) . ': ' . \print_r($event, \true));
        } else {
            $d = \is_array($event['data']) ? $event['data'][0] : null;
            $this->assertEquals($expects, $d, "Event {$type} should equal {$expects}: " . \print_r($event, \true));
        }
    }
    /**
    ...
    {
        $this->assertEquals('error', $event['name'], 'Expected error for event: ' . \print_r($event, \true));
    }
    /**
    ...
    protected function isAllGood($name, $depth, $tests, $debug = \false)
    {
        foreach ($tests as $try => $expects) {
            if ($debug) {
                \fprintf(\STDOUT, "%s expects %s\n", $try, \print_r($expects, \true));
            }
            $e = $this->parse($try);
            if ($depth > 0) {
                $this->assertEquals($depth, $e->depth(), "Expected depth {$depth} for test {$try}." . \print_r($e, \true));
            }
    ...
    {
        $bogus = array('</+this is a bogus comment. +>', '<!+this is a bogus comment. !>', '<!D OCTYPE foo bar>', '<!DOCTYEP foo bar>', '<![CADATA[ TEST ]]>', '<![CDATA Hello ]]>', '<![CDATA[ Hello [[>', '<!CDATA[[ test ]]>', '<![CDATA[', '<![CDATA[hellooooo hello', '<? Hello World ?>', '<? Hello World');
    ...
    public function testEndTag()
    {
      ...
      >' => 'test',
        ...
        $fail = array('</a class="monkey">' => 'a', '</a <b>' => 'a', '</a <b <c>' => 'a', '</a is the loneliest letter>' => 'a', '</a' => 'a');
    ...
    {
        $good = array('<!--easy-->' => 'easy', '<!-- 1 > 0 -->' => ' 1 > 0 ', '<!-- --$i -->' => ' --$i ', '<!----$i-->' => '--$i', "<!--\nHello World.\na-->" => "\nHello World.\na", '<!-- <!-- -->' => ' <!-- ');
        foreach ($good as $test => $expected) {
            $events = $this->parse($test);
            $this->assertEventEquals('comment', $expected, $events->get(0));
        }
        $fail = array('<!-->' => '', '<!--Hello' => 'Hello', "<!--\0Hello" => \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\UTF8Utils::FFFD . 'Hello', '<!--' => '');
    ...
    {
        $good = array('<![CDATA[ This is a test. ]]>' => ' This is a test. ', '<![CDATA[CDATA]]>' => 'CDATA', '<![CDATA[ ]] > ]]>' => ' ]] > ', '<![CDATA[ ]]>' => ' ');
        $this->isAllGood('cdata', 2, $good);
    }
    public function testDoctype()
    {
        $good = array('<!DOCTYPE html>' => array('html', 0, null, \false), '<!doctype html>' => array('html', 0, null, \false), '<!DocType html>' => array('html', 0, null, \false), "<!DOCTYPE\nhtml>" => array('html', 0, null, \false), "<!DOCTYPE\fhtml>" => array('html', 0, null, \false), '<!DOCTYPE html PUBLIC "foo bar">' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_PUBLIC, 'foo bar', \false), "<!DOCTYPE html PUBLIC 'foo bar'>" => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_PUBLIC, 'foo bar', \false), '<!DOCTYPE      html      PUBLIC     "foo bar"    >' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_PUBLIC, 'foo bar', \false), "<!DOCTYPE html \nPUBLIC\n'foo bar'>" => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_PUBLIC, 'foo bar', \false), '<!DOCTYPE html SYSTEM "foo bar">' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo bar', \false), "<!DOCTYPE html SYSTEM 'foo bar'>" => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo bar', \false), '<!DOCTYPE      html      SYSTEM "foo/bar"    >' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo/bar', \false), "<!DOCTYPE html \nSYSTEM\n'foo bar'>" => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo bar', \false));
        $this->isAllGood('doctype', 2, $good);
        $bad = array(
            '<!DOCTYPE>' => array(null, \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE    >' => array(null, \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE  foo' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE foo PUB' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE foo PUB>' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE  foo PUB "Looks good">' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE  foo SYSTME "Looks good"' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            // Can't tell whether these are ids or ID types, since the context is chopped.
            '<!DOCTYPE foo PUBLIC' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE  foo PUBLIC>' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE foo SYSTEM' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE  foo SYSTEM>' => array('foo', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_NONE, null, \true),
            '<!DOCTYPE html SYSTEM "foo bar"' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo bar', \true),
            '<!DOCTYPE html SYSTEM "foo bar" more stuff>' => array('html', \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack::DOCTYPE_SYSTEM, 'foo bar', \true),
        );
        foreach ($bad as $test => $expects) {
            $events = $this->parse($test);
            // fprintf(STDOUT, $test . PHP_EOL);
            $this->assertEquals(3, $events->depth(), "Counting events for '{$test}': " . \print_r($events, \true));
            $this->assertEventError($events->get(0));
    ...
    {
        $open = array('<foo>' => 'foo', '<FOO>' => 'foo', '<fOO>' => 'foo', '<foo >' => 'foo', "<foo\n\n\n\n>" => 'foo', '<foo:bar>' => 'foo:bar');
        $this->isAllGood('startTag', 2, $open);
        $selfClose = array('<foo/>' => 'foo', '<FOO/>' => 'foo', '<foo />' => 'foo', "<foo\n\n\n\n/>" => 'foo', '<foo:bar/>' => 'foo:bar');
        foreach ($selfClose as $test => $expects) {
            $events = $this->parse($test);
            $this->assertEquals(2, $events->depth(), "Counting events for '{$test}'" . \print_r($events, \true));
            $this->assertEventEquals('startTag', $expects, $events->get(0));
            $event = $events->get(0);
            $this->assertTrue($event['data'][2]);
        }
        $bad = array('<foo' => 'foo', '<foo ' => 'foo', '<foo/' => 'foo', '<foo /' => 'foo');
        foreach ($bad as $test => $expects) {
            $events = $this->parse($test);
            $this->assertEquals(3, $events->depth(), "Counting events for '{$test}': " . \print_r($events, \true));
            $this->assertEventError($events->get(0));
    ...
    {
        $cases = array('<id="top_featured">' => 'id', '<color="white">' => 'color', "<class='neaktivni_stranka'>" => 'class', '<bgcolor="white">' => 'bgcolor', '<class="nom">' => 'class');
    ...
    {
        $cases = array('<noscript<img>' => array('noscript', 'img'), '<center<a>' => array('center', 'a'), '<br<br>' => array('br', 'br'));
        foreach ($cases as $html => $expected) {
            $events = $this->parse($html);
            $this->assertEventError($events->get(0));
    ...
    {
        $cases = array('<li">' => 'li', '<p">' => 'p', '<b&nbsp; >' => 'b', '<static*all>' => 'static', '<h*0720/>' => 'h', '<st*ATTRIBUTE />' => 'st');
        foreach ($cases as $html => $expected) {
            $events = $this->parse($html);
            $this->assertEventError($events->get(0));
    ...
    public function testTagAttributes()
    {
        ...
        // Self-closing tags.
        $withEnd = array('<foo bar="baz"/>' => array('foo', array('bar' => 'baz'), \true), '<foo BAR="baz"/>' => array('foo', array('bar' => 'baz'), \true), '<foo BAR="BAZ"/>' => array('foo', array('bar' => 'BAZ'), \true), "<foo a='1' b=\"2\" c=3 d/>" => array('foo', array('a' => '1', 'b' => '2', 'c' => '3', 'd' => null), \true));
        $this->isAllGood('startTag', 2, $withEnd);
        // Cause a parse error.
        $bad = array(
            // This will emit an entity lookup failure for &+dark.
            "<foo a='blue&+dark'>" => array('foo', array('a' => 'blue&+dark'), \false),
    ...
    {
        $good = array('<script>abcd efg hijk lmnop</script>     ' => 'abcd efg hijk lmnop', '<script><not/><the/><tag></script>' => '<not/><the/><tag>', '<script><<<<<<<<</script>' => '<<<<<<<<', '<script>hello</script</script>' => 'hello</script', "<script>\nhello</script\n</script>" => "\nhello</script\n", '<script>&amp;</script>' => '&amp;', '<script><!--not a comment--></script>' => '<!--not a comment-->', '<script><![CDATA[not a comment]]></script>' => '<![CDATA[not a comment]]>');
        ...
        $bad = array('<script>&amp;</script' => '&amp;</script', '<script>Hello world' => 'Hello world');
        foreach ($bad as $test => $expects) {
            $events = $this->parse($test);
            $this->assertEquals(4, $events->depth(), "Counting events for '{$test}': " . \print_r($events, \true));
            $this->assertEventEquals('startTag', 'script', $events->get(0));
            $this->assertEventError($events->get(1));
    ...
    public function testRcdata()
    {
        ...
        $tok->setTextMode(\_PhpScoper833c86d6963f\Masterminds\HTML5\Elements::TEXT_RCDATA, 'title');
        $tok->parse();
    ...
    public function testText()
    {
        ...
        $this->assertEquals(4, $events->depth(), 'Events: ' . \print_r($events, \true));
        ...
        $this->assertEquals(7, $events->depth(), 'Events: ' . \print_r($events, \true));
        ...
        $this->assertEquals(2, $events->depth(), 'Events: ' . \print_r($events, \true));
    ...
    {
        $eventHandler = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\EventStack();
        $scanner = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Scanner($string);
        $scanner->debug = $debug;
        return array(new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Tokenizer($scanner, $eventHandler), $eventHandler);
    }
