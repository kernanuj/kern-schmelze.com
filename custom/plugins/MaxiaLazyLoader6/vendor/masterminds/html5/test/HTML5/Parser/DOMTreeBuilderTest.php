 ...
 */
class DOMTreeBuilderTest extends \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\TestCase
{
    ...
    {
        $treeBuilder = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\DOMTreeBuilder(\false, $options);
        $scanner = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Scanner($string);
        $parser = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Tokenizer($scanner, $treeBuilder);
        $parser->parse();
    ...
    {
        $treeBuilder = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\DOMTreeBuilder(\true);
        $scanner = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Scanner($string);
        $parser = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\Tokenizer($scanner, $treeBuilder);
        $parser->parse();
        $this->errors = $treeBuilder->getErrors();
    ...
    {
        $html = '<!DOCTYPE html><html></html>';
        $doc = $this->parse($html, array('disable_html_ns' => \true));
        $this->assertInstanceOf('\\DOMDocument', $doc);
        $this->assertEquals('html', $doc->documentElement->tagName);
    ...
    public function testDocumentFakeAttrAbsence()
    {
        ...
        $doc = $this->parse($html, array('xmlNamespaces' => \true));
        $xp = new \DOMXPath($doc);
    ...
    {
        $dom = $this->parse('<!DOCTYPE html><html><body><a t:href="bar">foo</a></body></html>', array('implicitNamespaces' => array('t' => 'http://www.example.com')));
        $a = $dom->getElementsByTagName('a')->item(0);
        $attr = $a->getAttributeNode('t:href');
        $this->assertEquals('http://www.example.com', $attr->namespaceURI);
        $dom = $this->parse('<!DOCTYPE html><html><body><t:a>foo</t:a></body></html>', array('implicitNamespaces' => array('t' => 'http://www.example.com')));
        $list = $dom->getElementsByTagNameNS('http://www.example.com', 'a');
        $this->assertEquals(1, $list->length);
    }
    public function testXmlNamespaces()
    {
        $dom = $this->parse('<!DOCTYPE html><html>
            <t:body xmlns:t="http://www.example.com">
                <a t:href="bar">foo</a>
            </body>
            <div>foo</div>
          </html>', array('xmlNamespaces' => \true));
        $a = $dom->getElementsByTagName('a')->item(0);
        $attr = $a->getAttributeNode('t:href');
        $this->assertEquals('http://www.example.com', $attr->namespaceURI);
    ...
    public function testInstructionProcessor()
    {
        ...
        $treeBuilder = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Parser\DOMTreeBuilder();
        $is = new \_PhpScoper833c86d6963f\Masterminds\HTML5\Tests\Parser\InstructionProcessorMock();
