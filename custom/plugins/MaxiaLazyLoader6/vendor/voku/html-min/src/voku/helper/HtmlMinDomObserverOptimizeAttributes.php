 ...
 */
final class HtmlMinDomObserverOptimizeAttributes implements \_PhpScoperfd240ab1f7e6\voku\helper\HtmlMinDomObserverInterface
{
    /**
     ...
     */
    private static $executableScriptsMimeTypes = ['text/javascript' => '', 'text/ecmascript' => '', 'text/jscript' => '', 'application/javascript' => '', 'application/x-javascript' => '', 'application/ecmascript' => ''];
    /**
    ...
    public function domElementAfterMinification(\_PhpScoperfd240ab1f7e6\voku\helper\SimpleHtmlDomInterface $element, \_PhpScoperfd240ab1f7e6\voku\helper\HtmlMinInterface $htmlMin)
    {
        ...
        foreach ((array) $attributes as $attrName => $attrValue) {
            ...
            if ($htmlMin->isDoMakeSameDomainsLinksRelative()) {
                ...
                foreach ($localDomains as $localDomain) {
                    ...
                    if (($attrName === 'href' || $attrName === 'src' || $attrName === 'srcset' || $attrName === 'action') && !(isset($attributes['rel']) && $attributes['rel'] === 'external') && !(isset($attributes['target']) && $attributes['target'] === '_blank') && \stripos($attrValue, $localDomain) !== \false) {
            ...
            if ($htmlMin->isDoRemoveHttpPrefixFromAttributes()) {
                $attrValue = $this->removeUrlSchemeHelper($attrValue, $attrName, 'http', $attributes, $tagName, $htmlMin);
            }
            if ($htmlMin->isDoRemoveHttpsPrefixFromAttributes()) {
                $attrValue = $this->removeUrlSchemeHelper($attrValue, $attrName, 'https', $attributes, $tagName, $htmlMin);
            }
            // -------------------------------------------------------------------------
        ...
        if ($htmlMin->isDoSortHtmlAttributes()) {
            ...
            foreach ($attrs as $attrName => $attrValue) {
                $attrValue = \_PhpScoperfd240ab1f7e6\voku\helper\HtmlDomParser::replaceToPreserveHtmlEntities($attrValue);
                $element->setAttribute((string) $attrName, $attrValue, \true);
            }
        }
    }
    /**
     ...
     * @return bool
     */
    private function removeAttributeHelper($tag, $attrName, $attrValue, $allAttr, \_PhpScoperfd240ab1f7e6\voku\helper\HtmlMinInterface $htmlMin) : bool
    {
        // remove defaults
     ...
     */
    private function removeUrlSchemeHelper(string $attrValue, string $attrName, string $scheme, array $attributes, string $tagName, \_PhpScoperfd240ab1f7e6\voku\helper\HtmlMinInterface $htmlMin) : string
    {
        /** @noinspection InArrayCanBeUsedInspection */
        if (!(isset($attributes['rel']) && $attributes['rel'] === 'external') && !(isset($attributes['target']) && $attributes['target'] === '_blank') && ($attrName === 'href' && (!$htmlMin->isdoKeepHttpAndHttpsPrefixOnExternalAttributes() || $tagName === 'link') || $attrName === 'src' || $attrName === 'srcset' || $attrName === 'action')) {
